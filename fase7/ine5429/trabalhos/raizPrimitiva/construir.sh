...
projeto=RaizPrimitiva
pacoteDoProjeto=raizPrimitiva
...
construir() {
	compilar;
	echo ":construir";
	cp -rf ${recursos}/documentos/${pacoteDoProjeto}.odt ${construcao};
	jar -cfe ${construcao}/${pacoteDoProjeto}.jar ${pacoteGeral}.${pacoteDoProjeto}.${projeto} -C ${binariosJava} .;
	zip -j ${construcao}/${pacoteDoProjeto}.zip ${construcao}/${pacoteDoProjeto}.odt ${construcao}/${pacoteDoProjeto}.jar;
}
